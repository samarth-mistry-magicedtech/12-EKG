...
namespace SB12.Editor
{
    public static class SB12SceneBuilder
    {
        ...
        private static void CreateUI(GameObject root)
        {
            ...
            if (panelGO.GetComponent<SB12.SlideController>() == null)
            {
                panelGO.AddComponent<SB12.SlideController>();
            }
        }
        ...
        private static void ApplyMaterialToNameMatches(Transform root, string key, Color color, params string[] nameTokens)
        {
            if (root == null || nameTokens == null || nameTokens.Length == 0) return;
            var mat = EnsureMaterialAsset(key, color);
            var tokens = new string[nameTokens.Length];
            for (int i = 0; i < nameTokens.Length; i++) tokens[i] = nameTokens[i].ToLowerInvariant();
            var rends = root.GetComponentsInChildren<Renderer>(true);
            for (int i = 0; i < rends.Length; i++)
            {
                var r = rends[i];
                ...
                {
                    if (n.Contains(tokens[t])) { match = true; break; }
                }
                if (!match) continue;
                var arr = r.sharedMaterials;
                if (arr != null && arr.Length > 1)
        ...
        private static void ApplyThemeMaterials(GameObject root)
        {
            ...
            if (patient != null)
            {
                ...
                if (genesisTf != null)
                {
                    ...
                    // Apply Body material explicitly to Genesis subtree as requested
                    SetRendererColor(genesisTf, "Body", patientCol, true);
                }
                // Apply pant material to pants and boots
                ApplyMaterialToNameMatches(patient, "LVA_Pant", new Color32(40,70,120,255), "pant", "pants", "boot", "boots", "sole", "shoe", "strap");
            }
            if (bed != null)
            {
                ...
                // Apply bed paper and bed cushion materials by name
                ApplyMaterialToNameMatches(bed, "Bed_PaperTXT", new Color32(245,245,245,255), "paper");
                ApplyMaterialToNameMatches(bed, "Bed_CushionTXT", new Color32(200,228,255,255), "cushion");
            }
        }
